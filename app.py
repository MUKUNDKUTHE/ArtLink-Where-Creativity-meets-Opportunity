...
def get_database_url() -> str:
    ...
    if not url:
        raise RuntimeError(
            "DATABASE_URL is not configured. Add it to Streamlit secrets or your environment."
        )
    return url
...
try:
    ...
    st.info("Set DATABASE_URL in Streamlit secrets before running this app.")
    st.stop()
c = CompatCursor(conn)

DatabaseOperationalError = psycopg.Error
...
def resolve_image_path(path_like, fallback=None):
    resolved = resolve_existing_path(path_like)
    if resolved:
        return str(resolved)
    fallback_path = resolve_existing_path(fallback)
    return str(fallback_path) if fallback_path else None
...
def image_to_streamlit_source(image_ref, fallback=None):
    resolved = resolve_image_path(image_ref, fallback)
    if not resolved:
        return None
    if is_data_uri(resolved):
        _, encoded = resolved.split(",", 1)
...
def get_live_activity(limit: int = 6):
    events = []
    try:
        recent_artists = c.execute(
            """
            SELECT full_name
            FROM artist_profile
        ...
        recent_bookings = c.execute(
            """
            SELECT client_name, artist_phone
            FROM bookings
        ...
        for row in recent_artists[:2]:
            events.append(f"Artist profile updated: {row['full_name']}")
        for row in recent_bookings[:2]:
            events.append(f"New booking request from {row['client_name']}")
        for row in recent_reviews[:2]:
            rating = int(row["rating"] or 0)
            if rating > 0:
                events.append(f"New {rating}-star review submitted")
            else:
                events.append("New review submitted")
    except Exception:
        return []
    return events[:limit]
...
def render_artist_spotlight_cards(limit: int = 6):
    ...
    for art in artists:
        ...
        resolved_img = resolve_image_path(img_path, "User.png")
        img_src = f"data:image/png;base64,{img_to_base64(resolved_img)}"
        ...
        short_desc = (description or "Artist profile available on ArtLink.")[:115]
...
def artist_dashboard_ui():
    ...
    with tab1:
        if profile and not st.session_state.edit_mode:
            ...
            profile_image = image_to_streamlit_source(img_path, "Artist.png")
            if profile_image:
                st.image(profile_image, width=160)
            else:
                st.image(image_to_streamlit_source("Artist.png"), width=160)
            ...
            if image_file is not None:
                ...
                current_image = image_to_streamlit_source(image_default)
                if current_image:
                    st.image(current_image, width=120, caption="Current Image")
    ...
    with tab2:
        ...
        if items:
            ...
            for i, item in enumerate(items):
                with cols[i % 3]:
                    item_image = image_to_streamlit_source(item["image_path"])
                    if item_image:
...
def admin_page():
    ...
    admin_user = _get_secret("ADMIN_USERNAME")
    admin_pass = _get_secret("ADMIN_PASSWORD")

    if not admin_user or not admin_pass:
        st.error("Admin login is not configured.")
        st.info("Set ADMIN_USERNAME and ADMIN_PASSWORD in Streamlit secrets before deploying.")
        return
...
def user_page():
    ...
    for i, artist in enumerate(artists):
        ...
        with cols[i % 2]:
            ...
            profile_image = image_to_streamlit_source(profile_img, "User.png")
            if profile_image:
                st.image(profile_image, width=95)
            else:
                st.image(image_to_streamlit_source("User.png"), width=95)
            st.markdown(
    ...
    with left:
        st.markdown(
            ...
            <div class="hero">
                ...
                <div class="hero-kpis">
                    <span class="kpi-chip"><strong>{snapshot['artists']}</strong> artist profiles live</span>
                    <span class="kpi-chip"><strong>{snapshot['bookings']}</strong> booking moments created</span>
                    <span class="kpi-chip"><strong>{snapshot['reviews']}</strong> reviews shared</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown(
            ...
            <div class='home-visual-card'>
                ...
                <div class='home-metrics-grid'>
                    <div class='hero-metric'><strong>{snapshot['artists']}</strong><span>Creative profiles ready to explore</span></div>
                    <div class='hero-metric'><strong>{snapshot['bookings']}</strong><span>Booking conversations captured</span></div>
                    <div class='hero-metric'><strong>{snapshot['reviews']}</strong><span>Community reviews shared</span></div>
...
def options_page():
    ...
    events = get_live_activity(limit=6)
    if events:
        components.html(
            f"""
            <div class="live-feed">
        ...
        st.markdown(
            "<div class='empty-state'><h4>No recent activity yet</h4><p>Live platform updates will appear here as artists, bookings, and reviews come in.</p></div>",
            unsafe_allow_html=True,
        )
